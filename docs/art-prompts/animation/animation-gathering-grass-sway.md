...
| Source asset | `crop.gathering_grass` / 聚灵草 |
| Video | `2s` at source `24 fps` |
| Extraction | `6 fps`, Godot playback `6 fps` |
| Canvas | `1024 × 1024` |
| Extracted image | `256 × 256` |
| Image budget | `12` forward images before seam trimming |
| Loop | `ping_pong` |
| Matte | `magenta` |
...
```text
Animate the supplied still image for asset "crop.gathering_grass" as one continuous 2-second 2D game-animation video.

Motion:
Move from the mature neutral pose toward one gentle harvest-pulse extreme: open the rounded leaves by a very small amount while a soft jade-green glow gathers from the leaf tips toward the root. End before the glow releases so mirrored playback returns naturally to the neutral state.
...
- the leaf count, stem structure, neutral silhouette identity, and viewing angle; allow only the requested gentle leaf, petal, fruit, or cap motion
- the original jade-green palette, rounded outlines, soft cel shading, and fixed light direction
...
- full growth stages, harvesting disappearance, fruit creation, leaf-count changes, or soil movement
- fast shaking, large deformation, detached leaves, invented particles, or matte contamination
...
- Extract exactly `12` forward images at `6` fps before loop processing; trim any tolerated excess duration and reject a short clip.
