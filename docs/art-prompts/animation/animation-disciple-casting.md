...
| Field | Value |
|---|---|
| Animation ID | `animation.disciple_casting` |
| Source asset | `character.disciple_spellcasting` / 弟子施展灵术动作母图 |
...
```text
Animate the supplied still image for asset "character.disciple_spellcasting" as one continuous 4-second 2D game-animation video.

Motion:
Complete one closed tending spell: the disciple raises one hand slowly, forms one small stable cyan-gold cultivation glyph, guides a thin light ribbon toward the fixed plant target, then lowers the hand and returns exactly to idle. Use restrained sleeve and hair follow-through with no combat stance.
...
- combat magic, lightning, fire, explosions, weapon summoning, teleportation, or large particle fields
- broken anatomy, extra fingers, costume changes, facial drift, changing glyph geometry, or matte contamination
