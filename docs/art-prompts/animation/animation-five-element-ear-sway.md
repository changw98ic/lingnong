...
| Field | Value |
|---|---|
| Animation ID | `animation.five_element_ear_sway` |
| Source asset | `crop.five_element_ear` / 五行穗 |
...
```text
Animate the supplied still image for asset "crop.five_element_ear" as one continuous 4-second 2D game-animation video.

Motion:
Complete one restrained five-color energy sequence through the existing grain heads in a fixed elemental order, returning exactly to the opening color state. Add only slight synchronized stalk sway and keep every grain head attached and structurally unchanged.
...
- the stalk count, grain-head count, attachment points, neutral silhouette identity, and viewing angle; allow only the requested gentle leaf, petal, fruit, or cap motion
- the original five-element palette, rounded outlines, soft cel shading, and fixed light direction
...
- random color order, neon colors, elemental explosions, full growth stages, harvest removal, or grain shedding
- large stalk bending, detached heads, structural drift, extra particles, or matte contamination
