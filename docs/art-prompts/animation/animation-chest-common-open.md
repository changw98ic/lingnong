...
| Field | Value |
|---|---|
| Animation ID | `animation.chest_common_open` |
| Source asset | `chest.common` / 普通宝箱 |
...
```text
Animate the supplied still image for asset "chest.common" as one continuous 3-second 2D game-animation video.

Motion:
Complete one closed reward cycle: begin with the common wooden chest closed, add one tiny anticipation compression with the base staying fixed, open the lid slowly, reveal a restrained warm jade light, then close and settle into the exact opening pose.
...
- the box proportions, hinge location, latch, trim, wood construction, closed-state identity, and viewing angle; allow the lid silhouette to change only through the existing hinge
- the original warm wood and jade palette, rounded outlines, soft cel shading, and fixed lighting
...
- loot objects flying out, coins, text, numbers, claim-button framing, repeated bouncing, or explosive glow
