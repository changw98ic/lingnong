...
| Field | Value |
|---|---|
| Animation ID | `animation.chest_elite_open` |
| Source asset | `chest.elite` / 精良宝箱 |
...
```text
Animate the supplied still image for asset "chest.elite" as one continuous 3-second 2D game-animation video.

Motion:
Complete one closed reward cycle: open the elite jade-trimmed chest with a gentle upward lid motion, reveal a soft cyan-blue inner glow and one small circular light wave, then close and return exactly to the opening pose. Keep the base and latch alignment stable.
...
- the box proportions, hinge location, latch, elite jade trim, closed-state identity, and viewing angle; allow the lid silhouette to change only through the existing hinge
- the original jade, wood, and cyan-blue palette, rounded outlines, soft cel shading, and fixed lighting
...
- loot objects flying out, text, numbers, claim-button framing, repeated bouncing, or large particle bursts
- detached lid, broken hinge, changing trim, box morphing, extra chests, or matte contamination
