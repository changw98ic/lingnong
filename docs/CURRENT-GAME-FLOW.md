# 《灵农修仙》当前游戏逻辑流程图

> 依据当前工作树中的可执行代码整理，核心规则以 `scripts/autoload/game_state.gd` 为准；UI 文案和旧设计说明不作为运行逻辑依据。

最新的目标玩法方向另见 [`docs/TARGET-GAME-FLOW.md`](TARGET-GAME-FLOW.md)，当前代码与目标版之间的差距不要混入本图。

## 1. 总流程

```mermaid
flowchart TD
    A[启动 Godot 项目] --> B[加载 main.tscn]
    B --> C[GameState._ready 初始化新局默认状态]
    C --> D[main._ready 调用 SaveManager.load_game]
    D --> E{存在 user://lingnong_save.json?}
    E -- 否 --> F[继续使用新局默认状态]
    E -- 是 --> G[读取 JSON 并恢复资源、灵田、丹药、境界、天赋、成就、熟练度、长期计数、事件和计时器]
    G --> H[按轮回/晋级/总收获计数结算离线天赋点与灵石差额]
    H --> I[写入离线领取账本]
    F --> J[构建 HUD 和 7 个标签页]
    I --> J
    J --> K[进入主循环]

    K --> L[每帧 main._process(delta)]
    L --> M[GameState.update_world(delta)]
    M --> N{已到大限?}
    N -- 是 --> O[生产、事件、自动化全部暂停]
    O --> P{玩家选择}
    P -- 商店买长生丹 --> Q[寿元 +30 年，解除大限]
    Q --> K
    P -- 开始新局 --> R[轮回次数 +1，清空本轮经营状态，保留天赋树、未用天赋点、累计修为、里程碑、熟练度和成就]
    R --> K
    P -- 出售现有资源/购买商店物品 --> S[仍可执行的非生产操作]
    S --> O

    N -- 否 --> T[扣除寿元]
    T --> U{本帧寿元归零?}
    U -- 是 --> O
    U -- 否 --> V[轮换季节、随机事件和噬金虫]
    V --> W{已解锁自动修炼?}
    W -- 是 --> X[按灵气浓度增加修为和灵气]
    W -- 否 --> Y[跳过自动修炼]
    X --> Z[成熟灵田自动收获并立即补种]
    Y --> Z
    Z --> AD[刷新面板和存档]
    AD --> K

    K -. 玩家点击任意面板 .-> AE[面板调用 GameState 明确方法]
    AE --> AF{校验通过?}
    AF -- 否 --> AG[返回失败反馈，不改状态]
    AG --> K
    AF -- 是 --> AH[修改统一运行状态]
    AH --> AI[state_changed]
    AI --> AI2[AchievementSystem.refresh：一次性解锁成就并增加成就点]
    AI2 --> AJ[所有面板和 HUD 刷新]
    AI2 --> AK[main 立即保存 JSON]
    AH -. 突破成功 .-> AL[realm_changed]
    AL --> AM[显示突破提示]
    AJ --> K
    AK --> K
    AM --> K

    K --> AN{累计运行 30 秒?}
    AN -- 是 --> AO[定时保存 JSON]
    AN -- 否 --> K
    AO --> K
```

## 2. 灵田、作物和资源循环

```mermaid
flowchart TD
    A[空闲且已购买的灵田] --> B[玩家选择已解锁作物]
    B --> C{plant_crop 校验}
    C -->|大限/锁定灵田/作物未解锁/非空| D[种植失败]
    C -->|通过| E[写入 crop_id、planted_at、ready_at]
    E --> F[成熟时间 =（基础生长时间 - 当前熟练度时间减免）÷ 生长倍率]
    F --> G[生长中]
    G --> H{玩家点击灵田?}
    H -- 是 --> I{仍在生长?}
    I -- 是 --> J[ready_at 减少 1 秒 × 点击天赋]
    J --> G
    I -- 否 --> K[等待自动收获]
    H -- 否 --> L{当前时间 >= ready_at?}
    L -- 否 --> G
    L -- 是 --> K
    K --> M{大限?}
    M -- 是 --> N[收获暂停]
    M -- 否 --> O[harvest_crop]
    O --> P[计算生产倍率]
    P --> Q[结算虫害、季节收获、灵气收获加成]
    Q --> R[按熟练度产量加成，直接结算修为 + 灵石]
    Q --> S[灵气保持不变]
    R --> T[累计该作物收获次数，跨门槛发放熟练度天赋点]
    T --> U[按当前生长倍率自动补种同一种作物]
    S --> T
    U --> G

    R --> U[进入境界、天赋和自动化成长链]
```

### 灵田生产计算

```text
生产倍率 = 境界生产倍率
         × 已解锁天赋生产倍率
         × 灵田档位倍率
         × 当前生产事件倍率
         × 狂暴丹 buff 倍率

作物数量 = max(1,
  floor(生产倍率 × 季节收获倍率 × 虫害因子 × 灵气收获倍率))
  + 当前熟练度产量加成

基础成熟时间 = max(0.001, 作物基础时间 - 当前熟练度成熟减免)

单轮修为 = 作物单株修为基值
         × 实际收获数量
         × 境界修炼倍率
         × 丹道/修为天赋倍率
         × 当前修炼事件倍率

单轮灵石 = 实际收获数量 × 作物灵石单价

单轮灵气 = 0
```

当前实际行为：作物成熟后只能由世界循环自动收获并补种；点击成熟作物不会进入手动收获分支，只会等待自动结算。

### 灵植熟练度

每种作物独立累计实际收获次数，进度跨大限和开始新局保留。产量和成熟时间取已达到的最高档位，天赋点在首次跨过门槛时累计：

| 门槛 | 聚灵草 | 凝神花 | 赤阳果 | 天道莲 |
|---:|---|---|---|---|
| 10 次 | 产量 +1，天赋 +1 | 同左 | 同左 | 同左 |
| 50 次 | 产量 +1、成熟 -1 秒、天赋 +2 | 产量 +1、成熟 -2 秒、天赋 +2 | 产量 +1、成熟 -5 秒、天赋 +2 | 产量 +1、成熟 -60 秒、天赋 +2 |
| 150 次 | 产量 +2、天赋 +3 | 同左 | 同左 | 同左 |
| 400 次 | 产量 +3、成熟 -1 秒、天赋 +5 | 产量 +3、成熟 -2 秒、天赋 +5 | 产量 +3、成熟 -5 秒、天赋 +5 | 产量 +3、成熟 -60 秒、天赋 +5 |

四种灵植共贡献 44 点熟练度天赋点。真实收获和模拟器都调用 `BalanceConfig.crop_proficiency_reward()` 与 `SimulationSystem.field_result()`，因此熟练度不会在两个模块中出现不同口径。

### 灵田购买和升档

| 操作 | 条件 | 消耗 | 结果 |
|---|---|---:|---|
| 购买第 2 块灵田 | 非大限、已有第 1 块 | 凡人基准 100；炼气/筑基/金丹为 300/800/2,000 | 已购槽位 +1 |
| 购买第 3 块灵田 | 非大限、已有第 2 块 | 凡人基准 500；炼气/筑基/金丹为 1,500/4,000/10,000 | 已购槽位 +1 |
| 凡田 → 灵田 | 炼气、非大限 | 4,000 灵石/块 | 该田产量档位 ×10 |
| 灵田 → 宝田 | 筑基、非大限 | 120,000 灵石/块 | 该田产量档位 ×100 |
| 宝田 → 仙田 | 金丹、非大限 | 3,000,000 灵石/块 | 该田产量档位 ×1,000 |

平衡口径：灵田是并行生产线，所以 1/2/3 块同档灵田的总产能是 `1× / 2× / 3×`；第三块相对前两块的边际提升是 `+50%`，不是再次翻倍。槽位价格随境界生产倍率同步缩放，保证第二块、第三块在四个境界的回本期不因境界跃迁而瞬间归零；农道核心天赋下第二块约 40~43 秒、第三块约 198~214 秒回本。升档按无农道约 120~180 秒、农道核心天赋约 45~90 秒回本，灵气库存、事件、狂暴丹和点击不纳入定价基线。

## 3. 修为循环

```mermaid
flowchart TD
    A[灵田收获] --> B[直接获得修为和灵石]
    B --> C[当前修为增加]
    C --> D[累计修为增加]
    D --> E{跨过修为里程碑?}
    E -- 是 --> F[每个新里程碑 +1 天赋点]
    E -- 否 --> G[等待下一轮收获]
    F --> G
```

修为只从灵田收获和自动修炼进入，不再经过炼丹服用。生产 buff（狂暴丹）通过商店购买获得，见“商店资源转换”。

## 4. 境界、天赋和大限

```mermaid
flowchart TD
    A[当前修为] --> B{达到下一境界门槛且未到大限?}
    B -- 否 --> C[继续种植、收获或自动修炼]
    C --> A
    B -- 是 --> D[玩家点击突破]
    D --> E[realm_index +1]
    E --> F[更新生产/修炼实时倍率]
    F --> G[寿元上限提高并立即补满]
    G --> H[按境界索引获得天赋点：炼气 3、筑基 7、金丹 15；晋级次数 +1]
    H --> I[应用该境界的内容解锁]
    I --> J[继续当前经营进度，不清空资源]
    J --> A

    H --> K[天赋点来源汇总]
    K --> L[突破奖励]
    K --> M[累计修为里程碑]
    K --> N[商店悟道残卷]
    L --> O[天赋树节点]
    M --> O
    N --> O
    O --> P{点数足够且前置满足?}
    P -- 否 --> Q[节点保持锁定]
    P -- 是 --> R[解锁节点，永久作用于当前存档和后续新局]
```

| 境界 | 突破所需当前修为 | 生产倍率 | 修炼倍率 | 寿元上限 | 突破天赋点 | 主要解锁 |
|---|---:|---:|---:|---:|---:|---|
| 凡人 | 0 | ×1 | ×1 | 60 年 | 0 | 聚灵草 |
| 炼气 | 200 | ×3 | ×3 | 120 年 | 3 | 灵雨诀、灵田升档至灵田 |
| 筑基 | 2,000 | ×8 | ×8 | 200 年 | 7 | 自动修炼、凝神花、庚金剑诀、守护灵阵 |
| 金丹 | 50,000 | ×20 | ×20 | 500 年 | 15 | 赤阳果、天道莲 |

默认寿元衰减为 0.5 年/秒，所以四个境界从满寿元到大限的理论时长约为 120 / 240 / 400 / 1,000 秒。长生印会进一步降低实际流逝速度。

### 天赋树结构

```mermaid
flowchart TD
    ROOT[道心初醒]
    ROOT --> FARM[农道入门]
    ROOT --> ALC[丹火入门]
    ROOT --> SPIRIT[灵根入门]

    FARM --> FY[丰收回响：生产 ×1.50]
    FARM --> FS[生长法门：生长速度 ×1.40]
    FY --> FC[天工收成：点击加速 ×1.50]
    FS --> FC

    ALC --> AP[丹火淬炼：灵田修为 ×1.60]
    ALC --> AQ[灵植品鉴：灵田修为 ×1.25]
    AP --> AC[丹道循环：灵田修为 ×1.80]
    AQ --> AC

    SPIRIT --> SQ[聚气潮汐：自动修炼 ×1.60]
    SPIRIT --> SL[长生印：寿元流逝 ×0.65]
    SQ --> SC[灵根化田：灵气收获加成 ×1.50]
    SL --> SC
```

### 大限和新局

```mermaid
flowchart TD
    A[运行时间持续扣寿元] --> B{寿元 <= 0?}
    B -- 否 --> A
    B -- 是 --> C[标记 lifespan_depleted = true]
    C --> D[update_world 立即返回]
    D --> E[自动种植/收获、自动修炼、事件处理暂停]
    E --> F{玩家处理方式}
    F -- 商店买长生丹且灵石 >=100 --> G[灵石 -100，寿元 +30，最高不超过上限]
    G --> A
    F -- 修炼·突破面板开始新局 --> H[轮回次数 +1，保留 talent_nodes、talent_points、total_cultivation_earned、里程碑索引、crop_proficiency、achievements、achievement_points 和长期计数]
    H --> I[清空灵石、灵气、当前修为、灵材、灵田、境界和本轮解锁]
    I --> J[新局回到凡人、60 年寿元、1 块已购灵田]
    J --> A
    F -- 出售已有灵材/虫尸或购买其它商店物品 --> K[非生产操作仍可执行]
    K --> C
```

## 5. 季节、随机事件、噬金虫和法术

```mermaid
flowchart TD
    A[世界循环每帧] --> B{经过 180 秒?}
    B -- 是 --> C[季节切换：春/夏/秋/冬]
    B -- 否 --> D[保持当前季节]
    C --> E[更新生长、收获或灵气的季节倍率]
    D --> F[检查随机事件冷却]
    E --> F

    F --> G{没有事件且冷却结束?}
    G -- 否 --> H{当前事件到期?}
    G -- 是 --> I[随机选择一个事件]
    I --> J[祥瑞降世：生产类 ×10，持续 60 秒]
    I --> K[天道感悟：修炼类 ×20，持续 30 秒]
    I --> L[兵主诞辰：噬金虫攻击次数 ×2、间隔减半，持续 60 秒]
    H -- 是 --> M[清空事件并重新等待 180 秒冷却]
    H -- 否 --> N[保持当前事件倍率]

    A --> O{有作物的灵田到虫害时间?}
    O -- 否 --> P[等待]
    O -- 是 --> Q[噬金虫进入活跃状态]
    Q --> R{再次到攻击时间?}
    R -- 否 --> Q
    R -- 是 --> S[攻击：先消耗守护灵阵次数]
    S --> T{阵法次数还有剩余?}
    T -- 是 --> U[本次攻击被阵法抵抗]
    T -- 否 --> V[虫害等级 +1，最高 3]
    U --> Q
    V --> Q

    Q --> W{筑基、20 灵气、剑诀不在冷却?}
    W -- 是 --> X[庚金剑诀：获得虫尸，清除该田虫害，进入 120 秒冷却]
    W -- 否 --> Q
    X --> Y[虫尸可按 8 灵石/个出售]
```

### 法术和阵法

| 操作 | 条件 | 状态变化 | 实际影响 |
|---|---|---|---|
| 灵雨诀 | 炼气解锁、10 灵气、目标灵田未生效 | 目标田 60 秒灵雨计时 | 后续种植的生长速度 ×8 |
| 庚金剑诀 | 筑基、20 灵气、目标田有活跃噬金虫、全局冷却结束 | 获得虫尸并重置该田虫害 | 清除噬金虫 |
| 升级守护灵阵 | 筑基、100 灵石 | 目标田阵法等级 +1，抵抗次数重置为 2+等级 | 减少后续虫害等级增长 |
| 出售虫尸 | 有虫尸 | 虫尸清零，灵石 +8/个 | 资源回流 |

## 6. 灵气、自动修炼和离线结算

```mermaid
flowchart TD
    A[已种植的已解锁灵田] --> B[灵气浓度 = 每块灵田 1 + tier 后求和]
    B --> C{筑基且自动修炼已解锁?}
    C -- 否 --> D[没有自动修炼收益]
    C -- 是 --> E[在线每帧按每秒速率结算]
    E --> F[修为/秒 = 浓度 ×0.5 ×境界修炼 ×天道感悟 ×自动修炼天赋]
    E --> G[灵气/秒 = 浓度 ×0.2 ×境界生产 ×祥瑞降世]
    F --> H[当前修为、累计修为、里程碑]
    G --> I[灵气库存]

    J[读取存档] --> K{寿元已耗尽?}
    K -- 是 --> L[保持未结算状态，等待续命]
    K -- 否 --> M[读取轮回次数、晋级次数和总收获次数]
    M --> N[按统一公式计算尚未领取的天赋点与灵石]
    N --> O[离线不增加修为和灵气]
    O --> P[更新离线领取账本并立即写回存档]
```

离线折算与离线时长无关：

```text
天赋点 = min(轮回次数, 3)
         + min(floor(晋级次数 / 2), 2)
         + min(floor(总收获次数 / 2000), 2)
灵石 = (轮回次数 × 50 + 晋级次数 × 100 + 总收获次数 × 2)
       × 当前境界生产倍率
```

天赋点总上限为 7；离线领取账本保证同一份长期计数不会重复领取。

灵气还会影响每次收获：每 100 点当前灵气提供 10% 收获加成，基础收获加成最高 ×3；灵根天赋可进一步放大这段加成。收获加成不会自动消耗灵气，灵雨诀和庚金剑诀会直接消耗灵气。

### 商店资源转换

| 商品 | 价格 | 效果 |
|---|---:|---|
| 长生丹 | 100 灵石 | 寿元 +30 年，不超过上限；大限时也能购买 |
| 聚气玉 | 80 灵石 | 灵气 +200 |
| 悟道残卷 | 500 灵石 | 天赋点 +1 |
| 狂暴丹 | 100 灵石起，每次购买后 ×1.5 递进（四舍五入到 10，不设上限） | 生产 ×3，持续 5 秒；连买延长持续时间 |

狂暴丹的购买次数跨大限新局保留，新游戏重置；大限期间不能购买。价格按累计购买次数计算，越买越贵，是灵石的长线消耗出口。

### 成就

成就是独立的长期目标，不直接进入修为、灵石和天赋点主经济。当前定义集中在 `BalanceConfig.ACHIEVEMENTS`，由 `AchievementSystem` 在 `GameState.emit_state_changed()` 中统一检查，完成记录和成就点跨大限、跨新局保留。

```mermaid
flowchart LR
    A[收获/修行/突破/买田/点天赋/开始新局] --> B[GameState.emit_state_changed]
    B --> C[AchievementSystem.refresh]
    C --> D{当前值达到目标?}
    D -- 否 --> E[保留进度，等待下一次状态变化]
    D -- 是且未完成 --> F[记录 achievement_id]
    F --> G[增加一次成就点]
    G --> H[成就面板显示完成]
    D -- 是且已完成 --> E
```

当前共 19 项、合计 49 成就点：

| 类别 | 目标 |
|---|---|
| 灵田 | 收获 1 / 100 / 1,000 次；购买第 2 / 3 块灵田 |
| 修行与境界 | 累计获得 1,000 / 1,000,000 点修为；达到炼气 / 筑基 / 金丹 |
| 熟练度 | 四种作物各达到 10 次；聚灵草和天道莲各达到 400 次 |
| 轮回与天赋 | 开始第 1 / 3 次新局；解锁 5 个天赋节点 |

成就点只用于展示长期完成度；成就面板列出每项目标、当前值、类别和奖励点数。

## 7. UI、状态同步和存档

```mermaid
sequenceDiagram
    participant U as 玩家
    participant P as 当前面板
    participant G as GameState
    participant S as SaveManager
    participant M as main.gd
    participant H as HUD/其它面板

    U->>P: 点击种植、突破、天赋、商店、成就或法术按钮
    P->>G: 调用一个明确的 GameState 方法
    G->>G: 校验寿元、境界、库存、前置、冷却和目标
    alt 校验失败
        G-->>P: false / 空结果
        P-->>U: 显示失败反馈
    else 校验成功
        G->>G: 修改资源、灵田、解锁或计时器
        G->>G: AchievementSystem.refresh，完成目标只解锁一次
        G-->>M: state_changed
        M->>S: 立即 save_game()
        M-->>H: 刷新 HUD、面板和浮动数字
        G-->>P: 成功结果
        P-->>U: 显示成功反馈
    end

    opt 突破成功
        G-->>M: realm_changed
        M-->>U: 显示突破提示
    end

    loop 每帧
        M->>G: update_world(delta)
        G-->>H: 时间敏感文本按面板节流刷新
    end

    opt 每 30 秒
        M->>S: save_game() 兜底保存
    end
```

### 数值模拟面板的数据联动

数值模拟不是另起一套配置或独立结算器。`GameState` 暴露统一入口，面板和 headless 探针都调用同一个 `SimulationSystem`：

```mermaid
sequenceDiagram
    participant U as 玩家
    participant P as 数值模拟面板
    participant G as GameState
    participant S as SimulationSystem
    participant C as BalanceConfig（唯一数值源）

    U->>P: 选择实时状态、境界、作物、四季、天赋、事件、突破流程或全量矩阵
    P->>G: get_simulation_report() / get_simulation_matrix()
    G->>S: 传入当前 GameState 与场景覆盖项
    S->>C: 读取统一境界、作物、灵田、天赋、配方、事件和自动化数值
    S->>S: 计算产量、周期、熟练度、灵气、槽位回本、升档回本、在线自动修炼、离线差额、丹药期望和点击突破流程
    S-->>G: 只读 Dictionary / Array 报告
    G-->>P: 刷新报告，不修改存档
    G-->>P: state_changed 后自动同步当前状态和成就进度
```

面板覆盖的情况：

- 实时状态：当前每块田的作物、档位、虫害和灵雨。
- 境界 × 灵田：凡人、炼气、筑基、金丹与 1/2/3 块田的总产能和槽位回本。
- 作物、四季、天赋、事件矩阵：使用 `BalanceConfig` 逐项比较。
- 熟练度矩阵：逐种作物比较 0 / 10 / 50 / 150 / 400 次档位。
- 境界总表：生产/修炼倍率、寿元、自动修炼和突破解锁。
- 突破流程：以点击频率、田数、档位和天赋路线为输入，按“种植 → 收获直接修为和灵石 → 突破”计算阶段耗时；当前默认 4 次/秒。
- 点击频率默认是玩家总点击数；面板也提供“每块灵田独立”模式，用于单独比较并行点击上限。
- 全量核心矩阵：境界 × 已解锁作物 × 四季 × 天赋路线 × 田数 × 可用档位。
- 可导出 `user://simulation_report.json`，用于记录当前参数下的完整结果。

成就面板：

- 展示 19 项成就的类别、说明、当前值、目标值、完成状态和成就点。
- 收获、修为、境界、灵田、熟练度、轮回和天赋节点变化后自动刷新。
- 成就点不参与生产、修炼、离线折算或天赋树消费；成就完成状态和成就点由 `SaveManager` 按存档版本 13 保存。

## 8. 当前运行状态的边界

### 每局会清空

- 灵石、灵气、当前修为。
- 虫尸；灵田收获直接结算灵石，不再经过出售步骤。
- 当前境界、已购买灵田槽位、灵田档位。
- 灵雨、阵法、虫害、随机事件、狂暴 buff、本轮解锁标志。

### 新局会保留

- 已点亮天赋节点。
- 未使用天赋点。
- 累计修为和已经领取到的里程碑位置。
- 每种灵植熟练度、晋级次数、轮回次数、总收获次数和离线领取账本。
- 成就完成状态和成就点。

### 当前实现中没有

- 额外的主动转世按钮或转世生产倍率；轮回次数只在大限后开始新局时递增，并用于离线折算。
- 手动收获模式切换。
- 作物成熟后的手动收获按钮。
- 离线自动收获、离线自动炼丹和离线噬金虫结算；离线只按长期计数结算天赋点与灵石。

## 9. 代码对应关系

| 逻辑 | 主要文件 |
|---|---|
| 启动、每帧循环、面板组合、即时/定时存档 | `scripts/ui/main.gd` |
| 所有运行状态、资源结算、突破、大限、事件、法术 | `scripts/autoload/game_state.gd` |
| JSON 保存、读取、离线结算入口 | `scripts/autoload/save_manager.gd` |
| 唯一玩法数值源 | `scripts/systems/balance_config.gd` |
| 境界门槛和解锁奖励规则 | `scripts/systems/realm_config.gd` |
| 作物解锁查询 | `scripts/systems/crop_config.gd` |
| 灵气浓度和在线自动修炼规则 | `scripts/systems/automation.gd` |
| 生产/修炼倍率连乘 | `scripts/systems/multiplier_stack.gd` |
| 天赋节点、前置关系和效果汇总 | `scripts/systems/talent_tree.gd` |
| 商店商品和价格 | `scripts/systems/shop.gd` |
| 成就定义、进度和一次性解锁 | `scripts/systems/achievement.gd`、`scripts/systems/balance_config.gd` |
| 交互面板、数值模拟面板和 HUD | `scripts/ui/panels/*.gd` |
| 真实收获与游戏内模拟共用的计算层 | `scripts/systems/simulation.gd` |

## 10. 已完成的运行检查

```bash
/opt/homebrew/bin/godot --headless --path . --editor --quit
/opt/homebrew/bin/godot --headless --path . --quit-after 2
/opt/homebrew/bin/godot --headless --path . --scene res://scenes/probes/field_balance_probe.tscn
git diff --check
```

以上命令均已通过；探针同时覆盖 `BalanceConfig` 驱动的真实收获、游戏内模拟入口、境界/作物/四季/天赋/事件/全量矩阵、成就自动解锁、熟练度、离线折算和狂暴丹递进价格。
