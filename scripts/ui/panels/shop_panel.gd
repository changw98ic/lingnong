...
func _ready() -> void:
	...
	_root_vbox.add_child(_make_label("出售灵材获得灵石，再把灵石投入寿元、灵气和天赋点，形成持续的资源回流。", 14, Color(0.72, 0.82, 0.76)))
	_items_box = VBoxContainer.new()
	_items_box.add_theme_constant_override("separation", 8)
	_root_vbox.add_child(_items_box)
	_build_rows()
...
func _refresh() -> void:
	for item in GameState.get_shop_items():
		...
		buy.text = "购买"
		buy.disabled = GameState.spirit_stones < cost or (item_id == "longevity_pill" and GameState.lifespan_years >= GameState.lifespan_max_years)
